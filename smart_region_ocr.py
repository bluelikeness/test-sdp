    ...
    def process_smart_regions(self, image_path):
        ...
        for i, region in enumerate(regions):
            ...
            try:
                ...
                # AI 처리 - tuple 반환값 올바르게 처리
                result_tuple = processor.process_image(temp_path, "shape_detection")
                
                # measure_time 데코레이터 때문에 (result, time) tuple이 반환됨
                if isinstance(result_tuple, tuple) and len(result_tuple) == 2:
                    result, process_time = result_tuple
                else:
                    result = result_tuple
                    process_time = 0
...
def test_smart_region_ocr():
    ...
    try:
        ...
        if result:
            ...
            result_path = "output/smart_region_result.txt"
            with open(result_path, 'w', encoding='utf-8') as f:
                f.write("=== 스마트 영역 OCR 결과 ===\n")
...
def main():
    ...
    if success:
        ...
        print("   output/smart_region_result.txt - 최종 결과")
